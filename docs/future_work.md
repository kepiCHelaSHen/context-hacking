...
~~**3. Council consensus detection (Layer 4 incomplete).**~~ **DONE** (April 2026).
`CouncilResult.consensus_issues` now detects cross-model agreement. Council blocks
builds in VALIDATION mode, is advisory in EXPLORATION, and consecutive drift triggers
EXIT 6. OpenAI council uses JSON response format.
...
~~**5. Crash recovery (`chp run --resume`).**~~ **DONE** (April 2026).
`--resume` reads state_vector.md (now includes streak counters) and continues from
last turn. Emergency state dump on crash/Ctrl+C. API retry with exponential backoff.

**6. Streaming telemetry dashboard.**
...
**7. `chp run --method cli` completion.**
...
**9. Standard benchmark evaluation.**
...
**10. Community experiment contributions.**
