...
st.set_page_config(
    page_title="CHP · Context Hacking Protocol",
    page_icon="⬡",
    layout="wide",
...
EXPERIMENT_CATALOG = {
    ...
    "spatial-prisoners-dilemma":  {"domain": "Game Theory",         "icon": "🎲", "catches": "async update, T/R/P/S payoff"},
    "lotka-volterra":             {"domain": "Ecology",             "icon": "🐺", "catches": "ODE params, zero extinction"},
    "sir-epidemic":               {"domain": "Epidemiology",        "icon": "🦠", "catches": "deterministic vs stochastic"},
    "ml-hyperparam-search":       {"domain": "Machine Learning",    "icon": "🤖", "catches": "grid search, data leakage"},
    "lorenz-attractor":           {"domain": "Chaos Theory",        "icon": "🦋", "catches": "Euler integration, wrong beta"},
    "quantum-grover":             {"domain": "Quantum Computing",   "icon": "⚛",  "catches": "classical search, boolean oracle"},
    "izhikevich-neurons":         {"domain": "Neuroscience",        "icon": "🧠", "catches": "Hodgkin-Huxley contamination"},
...
.stButton button {
    ...
    text-transform: uppercase !important;
    border-radius: 4px !important;
    border: 1.5px solid var(--ln2) !important;
    background: var(--sur) !important;
    color: var(--t2) !important;
...
.sc.g { border-top-color: var(--passbd); }
.sc.r { border-top-color: var(--failbd); }
.sc.a { border-top-color: var(--warnbd); }
...
.gm.p { border-left-color: var(--passbd); }
.gm.f { border-left-color: var(--failbd); }
.gm.w { border-left-color: var(--warnbd); }
...
.gm-score.p { color: var(--pass); }
.gm-score.f { color: var(--fail); }
.gm-score.w { color: var(--warn); }
.gm-track { height: 3px; background: var(--sur3); border-radius: 2px; position: relative; }
.gm-fill { height: 100%; border-radius: 2px; }
.gm-fill.p { background: var(--passbd); }
.gm-fill.f { background: var(--failbd); }
.gm-fill.w { background: var(--warnbd); }
...
.sr:nth-child(even) { background: var(--sur2); }
...
.pt tr:hover td { background: var(--bluebg); }
.pt tr.act td { background: var(--bluebg); }
.pt tr.act td:first-child { border-left: 3px solid var(--blue); }
...
def _log_entries(text: str, max_n: int = 15) -> list[str]:
    parts = re.split(r"\n---\n", text)
    return [p.strip() for p in parts if p.strip() and "## Turn" in p][-max_n:]
...
def _find_figures(experiment_dir: Path) -> list[Path]:
    d = experiment_dir / "figures"
    if not d.exists(): return []
    return sorted(
        [p for p in d.iterdir() if p.suffix.lower() in (".png", ".svg", ".jpg", ".jpeg")],
...
def _sh(label: str) -> str:
    return f'<div class="sh"><span class="sh-lbl">{label}</span><div class="sh-line"></div></div>'
...
def _gate_html(key: str, score: float) -> str:
    ...
    return (
        ...
        f'<span class="gm-score {cls}">{score:.2f}</span>'
        f'</div>'
        f'<div class="gm-track">'
        f'<div class="gm-fill {cls}" style="width:{pct}%"></div>'
        f'<div class="gm-tick" style="left:{thresh*100:.0f}%"></div>'
        f'</div>'
        f'<div class="gm-foot">threshold {thresh}</div>'
        f'</div>'
    )
...
def _mode_badge(mode: str) -> str:
    ...
    if "DONE" in m or "COMPLETE" in m:
        return '<span class="badge b-gn">✓ Complete</span>'
    if "EXPLORATION" in m:
        return '<span class="badge b-am">Exploration</span>'
    if "EXIT" in m:
        return '<span class="badge b-rd">Exit</span>'
...
def main() -> None:
    ...
    if experiment:
        ...
        if ed.exists():
            fig_count = len(_find_figures(ed))
    ...
    st.markdown(
        ...
        f'<div class="ctrl-stats">'
        f'T:<strong style="color:var(--t1)">{turn}</strong>'
        f'&nbsp;·&nbsp;{_mode_badge(mode)}'
    ...
    with b6:
        if st.button("⬇  Download All Figures", use_container_width=True, key="btn_dl"):
            ...
            if count > 0:
                st.download_button(
                    label=f"⬇ Save ZIP  ({count} figs · {total // 1024} KB)",
                    data=buf, file_name="chp_figures.zip",
                    mime="application/zip", key="btn_dl2", use_container_width=True,
                )
            else:
    ...
    tab_mon, tab_health, tab_report, tab_gallery = st.tabs([
        "📊 Live Monitor", "🏥 Protocol Health",
        "📄 Experiment Report", "🧪 Experiment Gallery",
    ])
    ...
    with tab_mon:
        ...
        with c2:
            st.markdown(f'<div class="sc"><div class="sc-lbl">Turn</div>'
                        f'<div class="sc-val">{turn}</div>'
                        f'<div class="sc-sub">of 50 max</div></div>', unsafe_allow_html=True)
        with c3:
        ...
        with left:
            ...
            if scores:
                ...
                st.markdown(_empty_box("awaiting build", "run chp run to start"), unsafe_allow_html=True)

            # Sigma gates
            st.markdown(_sh("σ-Gate Readings"), unsafe_allow_html=True)
            ...
            if drift:
                st.error("⚠ DRIFT flagged — re-read CHAIN_PROMPT.md before continuing")
            else:
        ...
        with right:
            ...
            if entries:
                for entry in reversed(entries):
                    ...
                    with st.expander(f"{'◆ ' if has_fp else ''}{label}",
                                     expanded=(has_fp or entry == entries[-1])):
                        st.markdown(entry[:2000])
            else:
                st.markdown(_empty_box("no turns recorded", "run chp run to start the protocol"),
                            unsafe_allow_html=True)
            ...
            if des:
                html = ('<div class="cw"><div class="ch">'
                        'Contraindications — do not repeat these approaches</div>')
                for i, de in enumerate(des):
                    rule = f'<div class="cr-rule">→ {de["rule"]}</div>' if de["rule"] else ""
                    html += (f'<div class="cr">'
                             f'<span class="cr-idx">C{i+1:02d}</span>'
                             f'<div><div class="cr-title">{de["title"]}</div>{rule}</div>'
                             f'</div>')
                html += '</div>'
                st.markdown(html, unsafe_allow_html=True)
            else:
            ...
            if experiment:
                ...
                if figs:
                    ...
                    st.markdown(_sh(f"Live Figures · {len(figs)} · {ago}"), unsafe_allow_html=True)
                    cols = st.columns(min(len(figs), 3))
                    for i, fig in enumerate(figs[:3]):
                        with cols[i]:
                            st.image(str(fig), use_container_width=True)
                            st.markdown(f'<div style="text-align:center;font-family:var(--mono);'
        ...
        if report_text:
            ...
            for section in re.split(r"\n## ", report_text):
                if "FALSE POSITIVE" in section.upper()[:30]:
                    ...
                    body  = "\n".join(lines[1:]).strip()
                    st.markdown(f'<div class="fp-box"><div class="fp-hdr">◆ False Positive Story</div>'
                                f'<div class="fp-body">{body}</div></div>', unsafe_allow_html=True)
                    break
                    ...
                    for i, fig in enumerate(figs):
                        with fc[i % 2]:
                            ...
                            st.markdown(f'<div class="fc-cap">{i+1}. {desc}</div></div>',
                                        unsafe_allow_html=True)
                    st.markdown(f'<div style="font-family:var(--mono);font-size:0.60rem;'
                                f'color:var(--t4);margin-top:4px;">'
                                f'source: experiments/{experiment}/figures/</div>',
                                unsafe_allow_html=True)
        else:
            st.markdown(_empty_box("no report generated yet",
                                   "complete the experiment to see results here"),
                        unsafe_allow_html=True)
    ...
    with tab_gallery:
        ...
        st.markdown(
            ...
            f'<table class="pt">'
            f'<thead><tr><th></th><th>Experiment</th><th>Domain</th>'
        ...
        if selected:
            ...
            exp_dir = next(
                (d for d in [project_dir / "experiments" / selected,
            ...
            if exp_dir:
                ...
                with dtabs[2]:
                    ...
                    if df.exists():
                        ...
                        if des2:
                            html = ('<div class="cw"><div class="ch">'
                                    'Contraindications for this experiment</div>')
                ...
                with dtabs[3]:
                    ...
                    if rp.exists():
                        ...
                        st.markdown(_empty_box("experiment not yet completed"),
                                    unsafe_allow_html=True)

                with dtabs[4]:
                    figs = _find_figures(exp_dir)
                    if figs:
                        st.markdown(f'<div style="font-family:var(--mono);font-size:0.68rem;'
                                    f'color:var(--t3);margin-bottom:10px;">'
                                    f'{len(figs)} figures · most recent: '
                                    f'{_time_ago(figs[0].stat().st_mtime)}</div>',
                                    unsafe_allow_html=True)
                        fc = st.columns(2)
                        for i, fig in enumerate(figs):
                            with fc[i % 2]:
                                ...
                                st.markdown(f'<div class="fc-cap">{desc}'
                                            f'<br><span style="color:var(--t4);">'
                                            f'{sz:.0f} KB · {mod}</span></div></div>',
                                            unsafe_allow_html=True)
                    else:
                        st.markdown(_empty_box("— no figures yet —",
    ...
    st.markdown(
        ...
        'style="color:var(--blue);">GitHub</a></div>',
        unsafe_allow_html=True,
    )
...
def _render_protocol_health(project_dir: Path) -> None:
    ...
    st.markdown(_sh("Protocol Health — Is the loop learning?"), unsafe_allow_html=True)
    st.markdown('<p style="font-size:0.78rem;color:var(--t3);margin-bottom:16px;">'
                'A healthy loop shows: declining drift rate, improving gate scores, '
                'faster turns, fewer fix cycles.</p>', unsafe_allow_html=True)
    ...
    AX = dict(
        gridcolor="#E2E5EF", linecolor="#E2E5EF",
        tickfont=dict(family="IBM Plex Mono", size=9, color="#9CA3AF"),
    )

    if not telemetry_path.exists():
        labels = [
            ("Tokens per Turn",      "—", "Fewer = more efficient"),
            ("False Pos. Caught",    "—", "n=3 false positives killed at n=10"),
            ("Dead Ends Avoided",    "—", "Each = a mistake not repeated"),
            ("First-Try Pass Rate",  "—", "Tests pass without fix cycle"),
            ("Total Lines Built",    "—", "Autonomous code generation"),
            ("Total Time",           "—", "Wall-clock loop duration"),
        ]
        ...
        st.markdown(_empty_box("awaiting telemetry",
                               "run chp run --method api to record"), unsafe_allow_html=True)
        return
    ...
    st.markdown(_sh("Trends Over Time"), unsafe_allow_html=True)
    ...
    if any(x > 0 for x in tokens):
        ...
        fig.add_trace(go.Scatter(x=turn_nums, y=tokens, name="tokens",
                                 mode="lines+markers", line=dict(color="#3B82F6", width=1.5)))
        ...
        for t in turns_data:
            if t.get("false_positive_caught"):
                fig.add_vline(x=t["turn"], line_color="#D97706", line_width=1, line_dash="dot")
        fig.update_layout(title="Tokens per turn", xaxis=AX, yaxis=AX, **WHITE_THEME)
    ...
    if any(x > 0 for x in g1):
        ...
        fig.add_trace(go.Scatter(x=turn_nums, y=g1, name="G1 Frozen",
                                 mode="lines+markers", line=dict(color="#10B981", width=1.5)))
        ...
        fig.update_layout(title="Critic gate scores", xaxis=AX, yaxis=AX, **WHITE_THEME)
        st.plotly_chart(fig, use_container_width=True)

    times = [t.get("duration_seconds", 0) for t in turns_data]
    if any(x > 0 for x in times):
        colors = ["#10B981" if x < 300 else "#F59E0B" if x < 600 else "#EF4444"
                  for x in times]
        fig = go.Figure()
        ...
        fig.add_trace(go.Scatter(x=turn_nums, y=[t / 60 for t in times], mode="markers",
                                 marker=dict(color=colors, size=8), showlegend=False))
        fig.update_layout(title="Time per turn (lollipop)", xaxis=AX, yaxis=AX, **WHITE_THEME)
        st.plotly_chart(fig, use_container_width=True)
    ...
    for t in turns_data:
        ...
        if t.get("false_positive_caught"):
            st.markdown(
                f'<div class="fp-box" style="padding:8px 12px;">'
                f'<div class="fp-hdr" style="margin-bottom:3px;">◆ Turn {n} — False Positive Caught</div>'
                f'<div class="fp-body" style="font-size:0.72rem;">'
                f'{t.get("false_positive_description", "see innovation log")}</div></div>',
                unsafe_allow_html=True,
            )
        if t.get("anomaly"):
            st.markdown(
                f'<div style="background:var(--failbg);border-left:3px solid var(--failbd);'
                f'padding:7px 12px;margin:3px 0;border-radius:0 4px 4px 0;'
                f'font-family:var(--mono);font-size:0.70rem;color:var(--fail);">'
